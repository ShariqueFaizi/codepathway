...
export default function BlogsPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const data = await getJson<{ posts: BlogPostListItem[] }>(
          "/api/blog-posts",
        );
        setPosts(data.posts ?? []);
      } catch (e) {
        ...
        <section className="relative py-16 lg:py-20 overflow-hidden">
          ...
          <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <p className="text-lg text-muted-foreground max-w-2xl">
              Practical guides, interview notes, and study plans—written to be skimmed and remembered.
            </p>
          </div>
        </section>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            {loading ? (
              ...
              <div className="text-center py-16">
                ...
                <p className="text-muted-foreground">Seed the database to populate blogs.</p>
              </div>
            ) : (
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                {posts.map((p) => (
